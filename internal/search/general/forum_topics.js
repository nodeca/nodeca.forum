...
module.exports = function (N, apiPath) {
  ...
  //
  N.wire.before('server:search.general.list', function register_forum_topic_search(env) {
    env.data.content_types.push('forum_topics');
  });
  ...
  N.wire.on(apiPath, function* execute_search(locals) {
    let results = yield N.search.execute([
      [
        ...
        'SELECT objectid FROM forum_topics WHERE MATCH(?) LIMIT 0',
