  ...
  N.wire.on('init:jobs', function register_forum_posts_rebuild() {
    N.queue.registerWorker({
      ...
      * map() {
        ...
        N.logger.info(`Start rebuilding forum posts (${chunks.length} chunks)`);
      ...
      * process() {
        ...
        N.logger.info(`Rebuilding forum posts ${this.data.from}-${this.data.to} - ${posts.length} found`);

        yield Promise.map(
          posts,
          post => N.wire.emit('internal:forum.post_rebuild', post._id),
          { concurrency: 50 }
        );

        N.logger.info(`Rebuilding forum posts ${this.data.from}-${this.data.to} - finished (${
      ...
      reduce(chunksResult) {
        ...
        N.logger.info('Finish rebuilding forum posts');
      }
    });
  });
};
