    ...
    N.queue.registerTask({
      ...
      async process(group_id) {
        ...
        await N.models.nntp.Group.update(
          { _id: group_id },
          { $set: { min_index: group.max_index + 1 } }
        );

        await N.models.nntp.Article.remove({ group: group_id });
        ...
        async function process_chunk(posts) {
          ...
          for (let i = 0; i < posts.length; i++) {
            ...
            bulk.insert({
              _id:      new mongoose.Types.ObjectId(Math.floor(post.ts / 1000)),
              source:   post._id,
              group:    group._id,
              index:    group.max_index - posts.length + i + 1
            });
          }
        ...
        await new Promise((resolve, reject) => {
          pump(
            N.models.forum.Post.find()
                .where('section').equals(group.source)
                .sort('_id')
                .where('_id').gte(start_post ? start_post._id : new mongoose.Types.ObjectId('000000000000000000000000'))
                .lean(true)
                .cursor(),
