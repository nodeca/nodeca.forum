...
module.exports = function (N, collectionName) {
  ...
  PostUrl.index({ url: 'hashed' });

  // used to fetch url chunks
  PostUrl.index({ rand: 1 });


  // Export statuses
  //
  PostUrl.statics.statuses = statuses;


  N.wire.on('init:models', function emit_init_PostUrl(__, callback) {
    N.wire.emit('init:models.' + collectionName, PostUrl, callback);
  });


  N.wire.on('init:models.' + collectionName, function init_model_PostUrl(schema) {
