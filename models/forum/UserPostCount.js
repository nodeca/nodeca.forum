// Keeping track of a number of posts made by a user in each section
//
...
module.exports = function (N, collectionName) {
  ...
  /*
   * Increment post counter by 1 for a user in a single section
   *
   * Params:
   *
   *  - user_id (ObjectId)
   ...
   */
  UserPostCount.statics.inc = async function inc(user_id, { section_id, is_hb }) {
    let data = await N.models.forum.UserPostCount.findOneAndUpdate(
      { user: user_id },
      {
        $inc: {
   ...
   */
  UserPostCount.statics.recount = async function recount(user_id, section_id = null) {
    let bulk_data;
  ...
  N.wire.on('init:models', function emit_init_UserPostCount() {
    return N.wire.emit('init:models.' + collectionName, UserPostCount);
  });


  N.wire.on('init:models.' + collectionName, function init_model_UserPostCount(schema) {
