...
var view = null;
var SELECTOR = '#rebuild-forum-posts-urls-task';
var ignore_runid = 0;
var last_runid = 0;
...
N.wire.on('navigate.done:admin.core.rebuild', function rebuild_forum_posts_urls_widget_setup() {
  if (!$(SELECTOR).length) { return; }

  var current = N.runtime.page_data.forum_posts_urls_task.current || 0;
  var total   = N.runtime.page_data.forum_posts_urls_task.total || 1;
  ...
  N.live.on('admin.core.rebuild.forum_posts_urls', update_task_status);
});


N.wire.on('navigate.exit:admin.core.rebuild', function rebuild_forum_posts_urls_widget_teardown() {
  if (!$(SELECTOR).length) { return; }

  view = null;
  ko.cleanNode($(SELECTOR)[0]);

  N.live.off('admin.core.rebuild.forum_posts_urls', update_task_status);
});


N.wire.once('navigate.done:admin.core.rebuild', function rebuild_forum_posts_urls_widget_setup_handlers() {
