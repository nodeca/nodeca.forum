...
var view = null;
var SELECTOR = '#rebuild-forum-topics-task';
var ignore_runid = 0;
var last_runid = 0;
...
N.wire.on('navigate.done:admin.core.rebuild', function rebuild_forum_topics_widget_setup() {
  ...
  var current = N.runtime.page_data.forum_topics_task.current || 0;
  var total   = N.runtime.page_data.forum_topics_task.total || 1;
...
N.wire.once('navigate.done:admin.core.rebuild', function rebuild_forum_topics_widget_setup_handlers() {
  ...
  N.wire.on(module.apiPath + '.start', function rebuild_start() {
    ...
    N.io.rpc('admin.core.rebuild.forum_topics.start')
      .then(function () {
  ...
  N.wire.on(module.apiPath + '.stop', function rebuild_stop() {
    ...
    N.io.rpc('admin.core.rebuild.forum_topics.stop')
      .then(function () {
        // reset progress bar to zero,
        // and ignore all updates on the last task
        ignore_runid = Math.max(prev_runid, ignore_runid);
